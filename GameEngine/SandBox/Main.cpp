...
int main()
{
	...
	world.AddUpdateSystem(Interface::ControlEditMode, 0);
	world.AddUpdateSystem(CameraMovement, 0);
	world.AddPostUpdateSystem(RandomCircleGenerator, 0);
	world.AddUpdateSystem(NeuralNetworkSystems::UpdateNeuralNetworkSystemV2, 0);
	world.AddUpdateSystem( NeuralNetworkSystems::ChangeNeuralNetworkColor, 2);
	world.AddUpdateSystem(NeuralNetworkSystems::ReproduceNuralNetworkSystem, 1);
	...
	for(int i = 0; i < width; ++i)
		for (int j = 0; j < height; ++j)
		{
			...
			if (rand() % 10 == 0)
			{
				world.Add<NeuralNetwork>(renderObject, NeuralNetwork::Create(3, { 2,4,4 }));
			}
		}
